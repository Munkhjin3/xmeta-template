...
export const Market = () => {
  ...
  return (
   ...
   <div className="flex gap-2 items-center">
    <Image src={item.img} className="size-6" alt="" sizes="100vw" width={0} height={0}/>
   <Typography className="!text-black">{item.title}</Typography>
   </div>
          <div className="flex gap-4">
            <Typography className="!text-black">{item.price}</Typography>
            <Typography className={cn(item.procent > 0 ?" text-green-600" : "text-red-600 font-semibold" )}>{item.procent > 0 && '+'}{item.procent}%</Typography>
          </div>
        </div>
      ))}
    </div>
